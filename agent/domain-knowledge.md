...
description: Business and domain context for agentsync.
source: authored
priority: 40
applies_to: ["*"]
tags: [domain, context]
...
`agentsync` is the shared, repo-local memory layer that fixes this. One edit in `agent/` → every supported tool's context file is regenerated.
...
- **`source: authored`** — Human-written. `agentsync sync` never touches these.
- **`source: generated`** — Machine-emitted (e.g., `stack.md`, `repo-map.json`). `agentsync scan` overwrites them.
- **`source: hybrid`** — Mixed. HTML-comment markers delimit generated blocks; human prose lives outside.
- **Drift** — When a user manually edits a generated file (`CLAUDE.md`, etc.) instead of the source in `agent/`. We detect via three-way checksum (source SHA, last-sync record, on-disk file). `agentsync sync` refuses to overwrite drifted files without `--force`; user manually copies their edits back.
...
3. **Plugin-extensible.** Adapters, scanners, linter rules are pluggable. `@agentsync/adapter-sdk` is the public contract.
...
- Not an LLM wrapper. The default install never calls an LLM. (`agentsync ai-draft` will land in M5.5 as a separately installed package.)
